...
contract TokenCache {
    ...
    function _cacheToken(DelegableMiniMeToken _token, address _owner) internal {
        tokenCache[_owner] = _token;
    }

    function _popTokenCache(address _owner) internal returns (DelegableMiniMeToken) {
        require(tokenCache[_owner] != address(0), ERROR_MISSING_TOKEN_CACHE);

        DelegableMiniMeToken token = DelegableMiniMeToken(tokenCache[_owner]);
