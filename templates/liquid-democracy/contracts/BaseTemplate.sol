...
contract BaseTemplate is APMNamehash, IsContract {
    ...
    */
    ...
    DelegableMiniMeTokenFactory internal miniMeFactory;
    ...
    constructor(DAOFactory _daoFactory, ENS _ens, DelegableMiniMeTokenFactory _miniMeFactory, IFIFSResolvingRegistrar _aragonID) public {
    ...
    */
    ...
    function _installVotingApp(Kernel _dao, DelegableMiniMeToken _token, uint64[3] memory _votingSettings) internal returns (DelegableVoting) {
    ...
    function _installVotingApp(
        ...
        internal returns (DelegableVoting)
    {
        bytes memory initializeData = abi.encodeWithSelector(DelegableVoting(0).initialize.selector, _token, _support, _acceptance, _duration);
        return DelegableVoting(_installNonDefaultApp(_dao, DELEGABLE_VOTING_APP_ID, initializeData));
    }

    function _installDelegableVotingApp(Kernel _dao, DelegableMiniMeToken _token, uint64[3] memory _votingSettings) internal returns (DelegableVoting) {
        return _installDelegableVotingApp(_dao, _token, _votingSettings[0], _votingSettings[1], _votingSettings[2]);
    }

    function _installDelegableVotingApp(
        Kernel _dao,
        DelegableMiniMeToken _token,
        uint64 _support,
        uint64 _acceptance,
    ...
    */
    ...
    function _installDelegableTokenManagerApp(
        Kernel _dao,
        DelegableMiniMeToken _token,
        bool _transferable,
        bool _delegable,
        ...
        internal returns (DelegableTokenManager)
    {
        DelegableTokenManager tokenManager = DelegableTokenManager(_installNonDefaultApp(_dao, DELEGABLE_TOKEN_MANAGER_APP_ID));
        _token.changeController(tokenManager);
        tokenManager.initialize(_token, _transferable, _delegable, _maxAccountTokens);
        return tokenManager;
    }
    ...
    function _createTokenManagerPermissions(ACL _acl, DelegableTokenManager _tokenManager, address _grantee, address _manager) internal {
        _acl.createPermission(_grantee, _tokenManager, _tokenManager.MINT_ROLE(), _manager);
        _acl.createPermission(_grantee, _tokenManager, _tokenManager.BURN_ROLE(), _manager);
    }

    function _createDelegableVotingPermissions(
        ACL _acl,
        DelegableVoting _voting,
        address _settingsGrantee,
        address _createVotesGrantee,
    ...
    // 
    function _mintTokens(ACL _acl, DelegableTokenManager _tokenManager, address[] memory _holders, uint256[] memory _stakes) internal {
        _createPermissionForTemplate(_acl, _tokenManager, _tokenManager.MINT_ROLE());
        for (uint256 i = 0; i < _holders.length; i++) {
            _tokenManager.mint(_holders[i], _stakes[i]);
        }
    ...
    function _mintTokens(ACL _acl, DelegableTokenManager _tokenManager, address[] memory _holders, uint256 _stake) internal {
        _createPermissionForTemplate(_acl, _tokenManager, _tokenManager.MINT_ROLE());
        for (uint256 i = 0; i < _holders.length; i++) {
            _tokenManager.mint(_holders[i], _stake);
        }
    ...
    function _mintTokens(ACL _acl, DelegableTokenManager _tokenManager, address _holder, uint256 _stake) internal {
        _createPermissionForTemplate(_acl, _tokenManager, _tokenManager.MINT_ROLE());
        _tokenManager.mint(_holder, _stake);
        _removePermissionFromTemplate(_acl, _tokenManager, _tokenManager.MINT_ROLE());
    }
    ...
    function _createToken(string memory _name, string memory _symbol, uint8 _decimals, bool _transfersEnabled, bool _delegationEnabled) internal returns (DelegableMiniMeToken) {
        require(address(miniMeFactory) != address(0), ERROR_MINIME_FACTORY_NOT_PROVIDED);
        DelegableMiniMeToken token = miniMeFactory.createCloneToken(DelegableMiniMeToken(address(0)), 0, _name, _decimals, _symbol, _transfersEnabled, _delegationEnabled);
        emit DeployToken(address(token));
        return token;
    }
